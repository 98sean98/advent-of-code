...
fn main() {
	...
	let mut stack: Vec<Light> = vec![(0, 0, 0)];
	mem[0][0] = (true, false, false, false);
	...
	let s = mem.iter().fold(0, |a, v| a + v.iter().fold(0, |b, w| if vec![w.0, w.1, w.2, w.3].iter().any(|&x| x) {b+1} else {b}));
