...
fn dist(p1: (usize, usize), p2: (usize, usize), e_rows: &Vec<usize>, e_cols: &Vec<usize>) -> u32 {
	let (min_r, max_r) = if p1.0 <= p2.0 { (p1.0, p2.0) } else { (p2.0, p1.0) };
	let (min_c, max_c) = if p1.1 <= p2.1 { (p1.1, p2.1) } else { (p2.1, p1.1) };
	let r = max_r - min_r;
	let c = max_c - min_c;

	let r_e = e_rows.iter().fold(0, |res, &a| if min_r < a && a < max_r { res + 1 } else { res });
	let c_e = e_cols.iter().fold(0, |res, &a| if min_c < a && a < max_c { res + 1 } else { res });
